...
@pytest.mark.parametrize("version", [-1, 0, 3, 99, None, True, "1"])
...
def test_current_writes_default_to_v2_and_cannot_choose_compatibility_format(model):
    fields = dict(office="SWT", name="Report", description="test", repo_path="report.py")
    assert model(**fields).config_version == 2
    for version in (1, 99):

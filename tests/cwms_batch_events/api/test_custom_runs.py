...
def test_run_and_upgrade_persists_schema_but_not_custom_arguments(client, user, job_queue, monkeypatch, admin):
    user.admin_offices = ["SWT"] if admin else []
    script = ScriptModel(**make_script_read(config_version=2, office="SWT", roles=[],
        repo_path="report.py", command_args=["--saved", "space "]).model_dump(by_alias=False))
    session = MagicMock()
    session.get_one.return_value = script
    session.refresh.side_effect = lambda job: setattr(job, "created_time", datetime.now(timezone.utc))
    monkeypatch.setattr("cwms_batch_events.core.job_database.postgres.postgres.get_runner_id", lambda: script.id)
    app.dependency_overrides[get_job_database] = lambda: PostgresJobDatabase(session)
    response = client.post("/jobs", json={"scriptId": str(script.id), "upgradeToVersion": 3,
                                         "commandMode": "shell", "shellCommand": "echo first && echo second"})
    if not admin:
        ...
        return
    assert response.status_code == 200
    assert response.json()["commandMode"] == "shell"
    assert script.config_version == 3
    assert script.command_args == ["--saved", "space "]
    assert script.command_mode == "arguments" and script.shell_command is None
    ordinary = client.post("/jobs", json={"scriptId": str(script.id)}).json()
    assert ordinary["configVersion"] == 3
    assert ordinary["commandArgs"] == ["--saved", "space "]

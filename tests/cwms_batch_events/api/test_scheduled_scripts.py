...
def test_scheduled_catalog_uses_authorized_catalog(client, job_db, user):
    due = make_script_read(
        config_version=4, schedule_enabled=True, schedule_type="hourly", schedule_minute=10
    )
    disabled = make_script_read(schedule_enabled=False)
    old = make_script_read(config_version=3, schedule_enabled=True, schedule_type="hourly", schedule_minute=10)
    job_db.retrieve_script_catalog.return_value = [due, disabled, old]

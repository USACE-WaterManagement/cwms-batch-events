...
test("office history shows colleagues, safe names, and manual/scheduled badges", async ({ page }, testInfo) => {
  ...
  await expect(results.getByRole("button", { name: /Reservoir report/ }).getByText("Manual", { exact: true })).toBeVisible();
  await expect(results.getByRole("button", { name: /Hourly update/ }).getByText("Scheduled", { exact: true })).toBeVisible();
  await expect(results.getByRole("button", { name: /Historical report/ }).getByText("Unknown", { exact: true })).toBeVisible();
  ...
  await results.getByRole("button", { name: /Reservoir report/ }).click();
  await expect(results.locator(".job-detail-fields").first()).toContainText("Jane Doe");
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  await page.screenshot({ path: testInfo.outputPath("office-history-mobile.png"), fullPage: true });
  await results.getByRole("link", { name: "Details", exact: true }).first().click();

...
export function ShareJob({ jobId }: { jobId: string }) {
  ...
  const share = async () => {
    try { await navigator.clipboard.writeText(url); setResult("copied"); }
  ...
  return <div className="min-w-0 space-y-2">
    ...
    {result === "copied" && <p role="status" className="text-sm text-green-800">Link copied. Recipients need access to this office.</p>}

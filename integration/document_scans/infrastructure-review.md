...
| District image | `infrastructure/config/job_config.py` maps SWT and other offices to the shared `wmes-job-runner` ECR repository. The district extension is implemented/tested, but publishing it under that shared reference would affect other districts' images. Do not overwrite the shared tag as an SWT-only deployment. Enabling without a job-definition mapping change requires the scanner to be deliberately included in the shared runner image; a district-specific image mapping remains deferred. |
